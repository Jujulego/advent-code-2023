...
pub trait Node<D: Clone + Eq> {
    ...
    fn get(&self, point: &Point2<i32>) -> Option<&D> {
        match self.child_holding(point) {
            Tree::Empty => None,
            Tree::Leaf(pt, data) => if point == pt { Some(data) } else { None },
            Tree::Node(child) => if child.area.holds(point) { child.get(point) } else { None },
        }
    }
