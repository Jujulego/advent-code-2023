...
use crate::quadtree::line_query::LineQuery;
...
impl<D: Clone + Eq, N: Node<D>> Quadtree<D, N> {
    ...
    pub fn has(&self, point: &Point2<i32>) -> bool {
        self.root.get(point).is_some()
    }

    #[inline]
    pub fn get(&self, point: &Point2<i32>) -> Option<&D> {
        self.root.get(point)
    }
...
impl<D: Clone + Eq> Quadtree<D, GlobalNode<D>> {
    ...
    pub fn column_query(&self, col: i32) -> LineQuery<D> {
        LineQuery::new(0, col, &self.root)
    }

    pub fn row_query(&self, row: i32) -> LineQuery<D> {
        LineQuery::new(1, row, &self.root)
    }
